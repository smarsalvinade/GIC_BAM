function  [x,y,utmzone] = deg2utm_zone(Lat, Lon, ZoneNumber) % Converts from Lat, Lon to UTM coordinates with a pre-established UTM zone.
% -------------------------------------------------------------------------
% [x,y,utmzone] = deg2utm(Lat,Lon, ZoneNumber) Modfied version to include ZoneNumber, a pre-established UTM zone!
%
...
%
% Outputs:
%    x (meters east), y (meters north), utmzone.   See example
%
% Example 1 (not updated for the last version):
...
%       51 R
%
% Example 2 (not updated for the last version): If you have Lat/Lon coordinates in Degrees, Minutes and Seconds
...
for i=1:n1
   ...
   if ZoneNumber < 1; ZoneNumber = fix( ( lo / 6 ) + 31); end
   S = ( ( ZoneNumber * 6 ) - 183 );
   deltaS = lon - ( S * ( pi / 180 ) );
   ...
   x(i)=xx;
   y(i)=yy;
   utmzone(i,:)=sprintf('%02d %c',ZoneNumber,Letra);
end
